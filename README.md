...
- 会话短期记忆：前端保存 `session_id`，后端用 MySQL `agent_conversation` / `agent_message` 持久化消息，并按最近 N 轮加载滑动窗口。
...
- 记忆：调用 `/api/memory`，查看长期记忆、按内容/类型/元数据搜索、手工写入、禁用、软删除、批量禁用、冲突提示和审计事件；对话气泡展示本轮使用/写入的记忆 ID。
- 管理员：登录为 `admin` 后可见，优先读取后端 MySQL 应用表的全局会话、工具调用和审计摘要；后端不可用时回退浏览器 `localStorage` 最近 100 轮问答，可查看路由、耗时、状态、工具调用、引用、图表和 Golden Set 评估摘要，并支持摘要 JSON 导出。
- 会话：前端保存当前 `session_id`，后端用 MySQL 应用表持久化消息，并按最近 N 轮加载短期滑动窗口；左侧历史会话列表可恢复和删除当前用户会话。

## 当前评估指标

当前公开指标来自本地私有 Golden Set 的 oracle 回放，用于验证评估管道、合约指标和前端展示闭环。原始 Golden Set 与 prediction JSONL 不提交远程。

| 指标 | 当前值 | 说明 |
| --- | --- | --- |
| 对话组数 | 100 | 覆盖 RAG、NL2SQL、报表、权限和记忆边界 |
| 总轮次 | 390 | 多轮对话 turn |
| SQL 期望轮次 | 140 | SQL 模板通过率 100% |
| RAG 证据轮次 | 110 | RAG Recall@5 为 100% |
| Prediction 数量 | 390 | oracle 回放覆盖全部轮次 |
| SQL 校验率 | 100% | 只读 AST 校验通过 |
| SQL 表召回 | 100% | expected_tables 命中 |
| SQL 过滤值召回 | 100% | expected_filters 命中 |
| 写操作违规 | 0 | 无 INSERT / UPDATE / DELETE / DROP |
| 需继承记忆轮次 | 280 | 验证多轮条件继承 |

完整展示口径见 [Phase5.9 评估展示报告](docs/Phase5.9-评估展示报告.md)。

## 后续规划

- [v1.1+ 记忆、评估与前端规划](docs/v1.1-记忆评估与前端规划.md)：补充短期记忆（滑动窗口）、长期记忆、Recall@K / MRR / NL2SQL 执行准确率等指标，以及前端管理员、记忆管理和评估看板路线图。
- [Phase5.9 评估展示报告](docs/Phase5.9-评估展示报告.md)：固化当前 Golden Set 指标表、失败案例分析口径和管理员看板截图清单。
...
# Golden Set 真实 Agent 小批量回放（默认 agent 模式 limit=5，依赖 LLM / Chroma / MySQL 在线）
uv run python scripts/replay_golden_set.py --mode agent --limit 5 --evaluate --summary-output data/eval_results/agent_smoke_summary.json

# 只回放某类场景，适合排查 NL2SQL / 记忆继承问题
uv run python scripts/replay_golden_set.py --mode agent --scenario-type nl2sql_condition_memory --limit 5 --evaluate
...
评估接口现在把排障线索统一放入 `traceHint`：`runId`、`sessionId`、`filters`、`copyText`、`traceUrl`、`searchUrl`。管理员前端会在评估运行历史和失败/风险样例中展示过滤键值，并提供复制过滤条件与打开 LangSmith/Trace 的操作。生产默认仍不在 LangSmith 或普通日志中保存完整真实问题、完整 SQL、完整检索片段；真实内容只允许受控调试场景短期保留。
