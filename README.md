...
- **工程可观测与可测试**：LangSmith 链路追踪、FastAPI SSE 流式输出、92 个 pytest 测试覆盖核心逻辑和外部依赖探活。
...
- 会话：前端保存当前 `session_id`，后端用本地 SQLite 持久化消息，并按最近 N 轮加载短期滑动窗口。
...
- 前端体验增强：补充后端会话列表、历史记录搜索、错误重试和更细的路由可视化。
