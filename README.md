...
- **工程可观测与可测试**：LangSmith 链路追踪、FastAPI SSE 流式输出、Golden Set 评估脚本和 94 个 pytest 测试覆盖核心逻辑。
...
- 端到端评估集：把 Golden Set prediction JSONL 接入真实 Agent 回放，输出 Recall@K / SQL 准确率 / Memory Hit Rate。
