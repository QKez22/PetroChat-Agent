...
> 本文是《PetroChat-Agent 项目结构说明书-Python版》的补充规划，面向 v1.0-multiagent 之后的演进。目标不是堆功能，而是把项目升级成一个能清楚说明工程闭环的 LLM 应用：有 Agent 编排、有记忆、有可量化评估、有前端观测和运维视角。

> 2026-07-07 更新：Phase 11 已把本文中的短期窗口、摘要记忆、长期记忆双层存储和 Mem0 候选/正式索引隔离推进到当前实现。实现细节、建表 SQL 和召回/写入流程见 [Phase11 上下文与长期记忆流程](Phase11-上下文与长期记忆流程.md)。
...
当前项目已经能演示“问答 + 工具 + 数据查询”，但如果要作为完整工程案例公开展示，需要补齐四个工程闭环：
...
4. 产品闭环：每一阶段都要有可演示入口、可测试脚本和 README 说明。
...
向量库中可保存 `user_memory.id`、`user_id`、`memory_type`、`content`、`created_at` 等元数据，正文仍以 MySQL 为准。
...
```text
Step E1: 读取私有 Golden Set，不提交原始数据
Step E2: 编写 scripts/eval_golden_set.py，输出数据集画像和合约指标（已完成基础版）
Step E3: 支持 prediction JSONL，计算 SQL / RAG / Memory 真实预测指标（已完成基础版）
Step E4: 编写 scripts/replay_golden_set.py，支持 oracle / agent 两种回放模式（已完成基础版）
Step E5: 把评估结果保存到 data/eval_results/*.json
Step E6: 增加 /api/evaluation/latest，前端管理员页读取真实评估摘要（已完成 API 基础版）
Step E7: 增加 /api/evaluation/failures，前端展示失败与风险样例（已完成摘要基础版）
Step E8: 增加 /api/evaluation/runs，前端展示评估历史和 LangSmith trace 查询线索（已完成基础版）
Step E9: 绑定真实 LangSmith trace URL 和评估运行批次
Step E10: README 展示关键指标和失败案例分析（已完成展示报告基础版）
```
...
| 引用展示 | 已完成 |
| 管理员本地观测 | 已完成 |
| 会话持久化 | 后端 MySQL 应用表 + 前端会话列表基础版 |
| 管理员评估摘要 | 后端 `/api/evaluation/latest` 聚合摘要 + 静态降级兜底基础版 |
| 失败风险样例 | 后端 `/api/evaluation/failures` 截断摘要 + 前端点选详情基础版 |
| 评估运行历史 | 后端 `/api/evaluation/runs` 本地摘要扫描 + Trace 查询线索基础版 |
...
1. 后端新增 conversation/message API。已完成
2. 前端左侧增加会话列表。已完成
3. 支持新建和删除会话。已完成基础版
4. 支持重命名、搜索会话。待增强
5. 支持导出会话 Markdown。待增强
6. 消息中保存 route、latency、citations、chart meta。部分完成
...
目标：展示系统不仅具备聊天入口，还具备 Agent 可观测性和运维视角。
...
1. 读取 `data/eval_results/*.json` 或后端 eval API。已接入 `/api/evaluation/latest` 基础版
2. 展示 RAG Recall@K、MRR、Faithfulness。已展示 RAG Recall@5、RAG MRR、证据覆盖率和忠实性代理指标
3. 展示 NL2SQL 执行准确率、安全违规率。已展示 SQL 校验率、表召回、过滤值召回、SQL 合约准确率，并预留结果级执行准确率字段
4. 展示 Memory hit rate、contradiction rate。已展示 Memory 合约统计
5. 支持失败样例回放：已展示截断输入、回答摘要、SQL/RAG 摘要和风险原因
6. 支持评估运行历史：已展示本地评估批次、产物状态和 LangSmith 查询线索；真实 trace URL 绑定待实现
...
1. 新增 `user_memory`、`memory_event` 表。
...
1. 基于私有 Golden Set 建立统一评估入口。已完成基础版
2. 实现 `scripts/eval_golden_set.py`，输出 SQL / RAG / Memory 合约指标。已完成基础版
3. 支持 prediction JSONL 评分。已完成基础版
4. 接入真实 Agent 批量回放，生成 prediction JSONL。已完成基础版
5. 输出 Recall@K / MRR / SQL 过滤值召回 / SQL 合约准确率 / Memory Hit Rate。
6. 在 README 写入最新指标表。

验收：

```text
uv run python scripts/eval_golden_set.py 输出 Golden Set 合约指标
uv run python scripts/eval_golden_set.py --predictions <jsonl> 输出预测指标
uv run python scripts/replay_golden_set.py --mode oracle --evaluate 输出 oracle prediction 指标
uv run python scripts/replay_golden_set.py --mode agent --limit 5 --evaluate 调用真实 Agent 小批量回放
```
...
| 阶段 | 目标 | 关键产出 | 状态 |
| --- | --- | --- | --- |
| Phase 5.9 | 评估报告固化 | README 指标表、失败案例分析、截图占位说明 | 已完成基础版 |
| Phase 6.1 | 长期记忆数据模型 | `user_memory` / `memory_event` 表结构与读写接口 | 已完成基础版 |
| Phase 6.2 | 长期记忆抽取 | 已升级为 Mem0 `infer=True` 自动候选抽取，经过 `MemoryPolicy` 后写入 MySQL | 已完成基础版 |
| Phase 6.3 | 长期记忆召回 | Agent 调用前按 `user_id` 召回 active 长期记忆，并与短期窗口、滚动摘要合并 | 已完成基础版 |
| Phase 6.4 | 记忆治理 | 前端记忆管理页、手工写入、删除/禁用/审计记录 | 已完成基础版 |
| Phase 7.1 | 真实 Agent 小批量回放 | agent 模式安全默认 limit、场景筛选、summary JSON、成功率/延迟指标 | 已完成基础版 |
| Phase 7.2 | 指标增强 | Memory Hit Rate、RAG MRR、证据覆盖率、忠实性代理指标、SQL 合约准确率，预留 SQL 执行准确率 | 已完成基础版 |
| Phase 7.3 | 失败归因 | 将失败样例归类为系统、路由、SQL、检索、记忆、模型输出和性能问题，并给出排查建议 | 已完成基础版 |
| Phase 7.4 | 评估报告导出 | Markdown/JSON 报告、趋势对比和公开展示指标 | 待做 |
| Phase 8.1 | 管理员后端化 | 管理员会话、工具调用、系统审计从 MySQL 应用表读取，前端保留本地 fallback | 已完成基础版 |
| Phase 8.2 | 前端记忆管理增强 | 记忆搜索、规则版冲突提示、批量禁用治理和前端选择状态 | 已完成基础版 |
| Phase 8.3 | Trace 深链接 | 评估运行与 LangSmith run URL / trace id 绑定 | 待做 |
| Phase 8.4 | Docker 演示环境 | 前端、FastAPI、Chroma、MySQL 一键启动 | 已完成基础版 |
| Phase 9.1 | 权限生产化 | 签名 token、过期时间、Bearer Header、PBKDF2 密码哈希兼容、管理员接口守卫 | 已完成基础版 |
| Phase 9.2 | 数据保留任务 | 会话、消息、工具日志、可选 RAG 上下文、审计日志和长期记忆过期治理 | 已完成基础版 |
| Phase 10.1 | 评估质量门禁 | `quality_gate`、`--fail-on-gate`、API `qualityGate` 和管理员门禁展示 | 已完成基础版 |
| Phase 10.2 | 真实 Agent 回放基线 | 场景配额选择、plan-only、`--execute-agent`、脱敏报告和本地 prediction 产物 | 已完成基础版 |
...
| `feat(phase-5.3): 增加 Golden Set 评估脚本` | Phase 5.3 | 指标基础版 |
| `feat(phase-5.4): 增加 Golden Set 回放脚本` | Phase 5.4 | prediction JSONL |
| `feat(phase-5.5): 增加管理员评估看板` | Phase 5.5 | 前端指标展示 |
| `feat(phase-5.6): 增加评估结果 API` | Phase 5.6 | 后端评估摘要接口 + 前端 API 接入 |
| `feat(phase-5.7): 增加评估失败样例回放` | Phase 5.7 | 失败/风险样例摘要接口 + 前端点选详情 |
| `feat(phase-5.8): 增加评估历史和 Trace 对照` | Phase 5.8 | 评估运行历史 + LangSmith 查询线索 |
| `docs(phase-5.9): 固化评估展示报告` | Phase 5.9 | 指标表 + 失败分析口径 + 截图清单 |
| `feat(phase-6.1): 增加长期记忆数据模型` | Phase 6.1 | user_memory / memory_event + 显式 API |
| `feat(phase-6.2): 接入长期记忆召回与保守写入` | Phase 6.2/6.3 | Agent state 记忆上下文 + memory_used/memory_written 元数据 |
| `feat(phase-6.4): 增加前端长期记忆治理页` | Phase 6.4 | 记忆查看、手工写入、禁用、删除和审计事件 |
| `test(phase-7.1): 增强真实 Agent 小批量回放` | Phase 7.1 | agent smoke replay + summary JSON + 成功率/延迟指标 |
| `test(phase-7.2): 增强 Golden Set 预测质量指标` | Phase 7.2 | Memory Hit Rate + RAG MRR + 证据覆盖 + SQL 合约准确率 + API 聚合展示 |
| `feat(phase-7.3): 增加评估失败归因` | Phase 7.3 | failures API 归因汇总 + 单条 primaryAttribution/attributions + 前端归因展示 |
| `feat(phase-8.1): 管理员观测台接入后端数据` | Phase 8.1 | admin 只读 API + Vue 后端观测区 + MySQL 会话/工具/审计摘要 |
| `feat(phase-8.2): 增强前端记忆治理` | Phase 8.2 | memory 搜索 + 冲突提示 + 批量禁用 + 前端多选治理 |
...
这样每一步都能单独 commit、单独演示，也能形成清晰的技术演进记录。

## Phase 8.3 Trace 定位补充

- 状态：已完成基础版。
- 后端：`/api/evaluation/failures` 与 `/api/evaluation/runs` 的 `traceHint` 统一返回 `runId`、`sessionId`、`filters`、`copyText`、`traceUrl`、`searchUrl`。
- 前端：管理员评估看板在“评估运行历史”和“失败/风险样例”中展示过滤条件标签，支持复制 `copyText`，并可打开 LangSmith 入口或真实 `traceUrl`。
- 边界：默认不把完整真实问题、完整 SQL、完整检索片段写入 traceHint；真实 Trace 链接仅在回放产物显式提供 `trace_url/run_url` 时展示。
- 建议提交：`feat(phase-8.3): 增加评估 Trace 定位包`。

## Phase 8.4 Docker 演示环境补充

- 状态：已完成基础版。
- Compose：新增 `mysql`、`chroma`、`api`、`frontend` 四服务编排，支持 `docker compose up -d --build` 一键启动。
- MySQL：新增 demo 初始化 SQL，包含 `user`、`affair`、`affair_task`、会话、工具日志、审计日志和长期记忆表。
- 前端：新增 Nginx 容器，代理 `/api`、`/health`、`/config` 到 FastAPI，并关闭 SSE 代理缓冲。
- 文档：新增 `docs/Phase8.4-Docker演示环境.md`，说明启动、入库、健康检查、账号和清理命令。
- 建议提交：`chore(phase-8.4): 增加 Docker 一键演示环境`。

## Phase 9.1 权限生产化补充

- 状态：已完成基础版。
- 后端：登录后签发 HMAC-SHA256 签名的 JWT 风格 token，包含 `iat/exp`。
- 鉴权：`/api/auth/me` 和 `/api/admin/*` 支持 `Authorization: Bearer <token>`，兼容旧 query token。
- RBAC：管理员接口统一走 `require_admin`，工程师用户访问返回 `403`。
- 密码：支持 `pbkdf2_sha256$...` 哈希格式，开发环境兼容当前明文密码。
- 工具：新增 `scripts/hash_password.py` 生成密码哈希。
- 建议提交：`feat(phase-9.1): 增加签名认证与管理员鉴权`。

## Phase 9.2 数据保留任务补充

- 状态：已完成基础版。
- 后端：新增 `RetentionCleanupService`，按策略统计并清理应用表。
- 命令：新增 `scripts/cleanup_retention.py`，默认 dry-run，显式 `--execute` 后才修改数据库。
- 策略：会话 180 天软删除、30 天恢复期后物理删除；工具日志 365 天；审计日志 1095 天；长期记忆到期禁用并写 `memory_event`。
- 审计：正式执行会写入 `agent_audit_log.action_type=retention_cleanup`，记录操作者、范围、原因和影响数量。
- 建议提交：`feat(phase-9.2): 增加数据保留清理任务`。
