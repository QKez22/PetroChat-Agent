...
| RAG MRR | 已接入 | 真实 prediction 回放后按首个命中证据排名计算 |
...
> 说明：当前公开指标来自 oracle 回放，表示评估管道、标注合约和前端展示闭环已打通；Phase 7.1 已提供 `--mode agent` 小批量真实回放入口，真实模型质量需要在本地依赖全部在线后生成新的 prediction 指标再更新。
...
Phase 6 已进入长期记忆。Phase 6.1 完成 MySQL 数据模型后，Phase 6.2/6.3 基础版已经接入调用前召回和规则版保守写入：

1. 从显式用户表达中抽取偏好和常用业务条件。
2. 为每条候选记忆生成来源、置信度和写入触发元数据。
3. 在 Agent 调用前合并短期滑动窗口与长期记忆。
4. 下一步在管理员或用户前端提供记忆查看、禁用、删除和审计入口。

## Phase 8.3 Trace 定位补充

失败/风险样例现在不仅给出归因类型和下一步建议，还返回 `traceHint` 定位包。管理员可按 `run_id`、`session_id`、`dialogue_id`、`turn_id` 过滤 LangSmith trace；如果回放产物提供真实 `trace_url/run_url`，前端会直接展示打开入口。该设计把“指标异常 -> 失败样例 -> Trace 调用链”串成闭环，同时避免在普通日志或前端接口中暴露完整真实问题、完整 SQL 和完整检索片段。
